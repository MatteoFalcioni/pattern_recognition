...
If you want to plot the stored data, you must launch the file plot.py. This will plot not only the losses saved in [toplot](toplot) but also the comparison between the efficiency of the three models (stored in the [efficiency](efficiency) file), which is based on computing the hamming distance and the cosine similarity between the generated text, and by measuring the accuracy of the prediction on the validation set. 
