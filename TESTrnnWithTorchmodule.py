...
"""def initialize_seq(train=True):  # initialize inputs and targets sequences
    ...
    # print(f'inputs: input text: {fulltext[:50]}, encoded input: {inputs[:50]},\n targets: {targets[:3]}')
...
class RNN(nn.Module):
    ...
    def forward(self, x):
        ...
        # 3d hidden state of zeros, we have batches (for training or validation)
...
""" # shuffle batches before each epoch:
    idx_tr = torch.randperm(Xtr.size(0))
    idx_ev = torch.randperm(Xev.size(0))
    Xtr = Xtr[idx_tr, :, :, :]
    Ytr = Ytr[idx_tr, :, :]
    Xev = Xev[idx_ev, :, :, :]
    Yev = Yev[idx_ev, :, :]"""
...
# if I sample through a multinomial with softmax, will it be right?
